...
def analyze_thumbnail(image_path, niche_mode="Technical"):
    ...
    return {
        "score": final_score, "contrast": round(contrast_score, 1), "sharpness": round(sharpness_score, 0),
...
def detect_boring_signals(video_path):
    ...
    try:
        ...
        return {
            "boring_score": boring_score, "stagnation_rate": round(stagnation_rate, 1), "avg_motion": round(avg_motion, 2),
...
if run_analysis or seo_only:
    if not title_input:
        ...
        if full_script_input:
            ...
            with st.spinner("Ruthlessly editing your script..."):
                compression_data = compress_script_with_llm(full_script_input, is_short)
            if "error" in compression_data:
                ...
                if is_short:
                    ...
                    if compression_data.get('compressed_word_count', 0) > 150:
                        st.error("⚠️ Still too long! Must be under 150 words for Shorts.")
                    else:
        ...
        if is_short and (run_analysis or seo_only):
            st.markdown("---")
            st.subheader("📱 Shorts SEO & Description")
        ...
        if title_input:
            ...
            with st.spinner("Analyzing title..."):
                title_analysis = analyze_title_with_llm(title_input, final_transcript, topic_input, is_short)
        ...
        if video_path and os.path.exists(video_path):
            ...
            if problem_input or mechanism_input or payoff_input:
                ...
                with st.spinner("Weaving your ingredients..."):
                    llm_data = analyze_script_with_llm(problem_input, mechanism_input, payoff_input, cpm, is_short)
